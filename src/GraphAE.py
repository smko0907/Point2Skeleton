...
class LinkPredNet(nn.Module):
    """
    Link Prediction Network Module
    NOTE:
    - Base class for neural network module (nn.Module) 
    - Deep GCN with 12 layers to capture richer structures at various levels of abstraction
    - existing works use a Shallow GCN; Usually no more than 4 layers
    - Apply Batch Normalization over 3D input without running estimates every time
    - Include residual blocks between consecutive layers to handle the degeneration problem

    GAE has two main jobs:
    1. The Encoder: It looks at each skeletal point and its neighbors to create a "summary" 
                    (mathematical description) of that point.
    2. The Decoder: It takes those summaries and predicts whether a connection (link) 
                    should exist between any two points.


    Input (Raw Data):
    - Features: Information about each point, such as its position (x, y, z), 
                radius and "shape features" that describe the local surface.
    - Initial Graph (A): A starting guess of how the points might be connected.
                        Generated by KNN 
    """
    def __init__(self):
    ...
    def encode(self, x, A):
        """
        The Encoder acts as a thinker with 12 layers of deep GCN.
        - Graph Convolutions: Unlike regular AI that looks at pixels in a grid, 
                            these layers "talk" to neighboring points in the graph to understand the structure.
        - Residual Blocks (The "Shortcuts"): You will notice lines like + self.align_2_3(x2) in the encode function. 
                                            These are "shortcuts" that allow the network to remember original 
                                            information even as it goes deeper into the 12 layers. This prevents 
                                            the network from getting "confused" or losing data as it processes.
        """
        x = x.transpose(1, 2)
        x_shape = x[:, 0:512, :]
        x_skel = x[:, 512:516, :]
    ...
    def compute_loss(self, A_recon, A_input, known_mask):
        """
        The `pos_weight` is calculated by dividing the number of non-edges (negatives) by the number of edges (positives) 
            -> "An error on a real connection is worth ~50x more than an error on an empty space"
        It forces the model to pay extra attention to the thin lines of the skeleton
        """
        known_nodes = known_mask.sum()
        pos_weight = float(known_nodes - A_input.sum()) / A_input.sum()
        # Logits: These are the raw, uncurbed scores coming out of the neural network (they can be any number, like -5.2 or 10.4).
        # The Benefit: Mathematically, combining the "Sigmoid" activation (which turns scores into 0-1 probabilities) and the "Cross Entropy" calculation into one step is more numerically stable. It prevents computer rounding errors that happen when dealing with very small probabilities (like 0.0000001).
        # The `weight=known_mask` parameter ensures the model only learns from valid parts of the data;
        # If some points in the pointcloud were "padded" or are invalid for the current shape, the known_mask is set to 0 for those spots. 
...
class InnerProductDecoder(nn.Module):
    """
    InnerProductDecoder acts like a "Matchmaker"
    It compares the final "summaries" of every pair of points.
    If two points have summaries that "match" well (mathematically, a high inner product), 
    the decoder predicts that they should be connected.
    """
